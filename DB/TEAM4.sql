...
CREATE TABLE USER_INFO(
    ...
    HAS_PARTNER VARCHAR2(1) CHECK(LOVER IN ('Y', 'N')),      -- 애인 유무
...
CREATE TABLE POST (
    ...
    POST_USER_ID VARCHAR2(50) NOT NULL,    -- 글 작성자 아이디(FK)
    PLACE_SEQ NUMBER,                      -- 지역 SEQ(FK)
    POST_THEME_SEQ NUMBER,                 -- 게시글 말머리 SEQ(FK)
    BOARD_SEQ NUMBER,                      -- 게시판 SEQ(매칭글인지, 리뷰글인지, 커뮤니티글인지 구분) (FK)
    MATCHING_SEQ NUMBER,                   -- 매칭 SEQ(리뷰글에 필요)(FK)
...
CREATE TABLE MATCHING (
    MATCHING_SEQ NUMBER PRIMARY KEY,        -- 매칭 SEQ(PK)
    POST_SEQ NUMBER,                       -- 글 SEQ(FK)
...
ALTER TABLE USER_PLACE_INFO ADD CONSTRAINT PLACE_USER_ID_FK FOREIGN KEY(USER_ID) REFERENCES USER_INFO(USER_ID);    -- 유저아이디(유저정보, 지역정보)
