...
DROP TABLE MATCHING_CANCEL_INFO CASCADE CONSTRAINT; -- 매칭 취소 정보 
...
DROP SEQUENCE SEQ_MATCHING_CANCEL_INFO; -- 매칭 취소 정보 SEQ
...
-- 매칭 취소 사유
CREATE TABLE MATCHING_CANCEL_INFO (
    MATCHING_CANCEL_SEQ NUMBER PRIMARY KEY,
    CANCEL_REASON VARCHAR2(3000),
    MATCHING_USER_INFO_SEQ NUMBER
);
...
ALTER TABLE MATCHING_CANCEL_INFO ADD CONSTRAINT MC_INFO_MU_INFO_SEQ_FK FOREIGN KEY(MATCHING_USER_INFO_SEQ) REFERENCES MATCHING_USER_INFO(MATCHING_USER_INFO_SEQ); -- 매칭유저정보SEQ(매칭유저정보, 매칭취소정보)
...
CREATE SEQUENCE SEQ_MATCHING_CANCEL_INFO; -- 매칭 참여 유저 정보 SEQ
